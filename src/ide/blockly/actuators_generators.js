...
export function registerActuatorGenerators(generator, boardId) {
    ...
    // --- JS Helper to parse Hex color (used locally) ---
...
`;
    ...
    // --- Generator for the Angle Shadow (Slider) Block ---
    // Even though it is called 'shadow', it now generates code for the slider field
    generator.forBlock['math_angle_shadow'] = function(block) {
...
`;
    ...
    generator.forBlock['actuator_servo_write'] = function(block) {
        ...
        // Accept input from Angle Slider, Math blocks, or Variables
        ...
        // Clamp values logic for safety
        const safeAngle = `max(0, min(180, ${angle}))`;

        if (boardId === 'pico') {
            return `${varName}.duty_u16(int(1638 + (${safeAngle} / 180) * 6554))\n`;
        } else {
            return `${varName}.duty(int(26 + (${safeAngle} / 180) * 102))\n`;
        }
    };
