...
class HCSR04:
    def __init__(self, trigger_pin, echo_pin, echo_timeout_us=30000):
        ...
        if isinstance(trigger_pin, int):
            ...
            self.trigger.init(mode=Pin.OUT, pull=None)
        if isinstance(echo_pin, int):
            self.echo = Pin(echo_pin, mode=Pin.IN, pull=None)
        else:
            self.echo = echo_pin
            self.echo.init(mode=Pin.IN, pull=None)
...
`;
        ...
        const tutorialFile = params.get('tutorial');
        const sharedProjectData = params.get('project_data');

        if (tutorialFile) {
            this.loadTutorialFromUrl(tutorialFile);
        }
...
`;
    ...
    switchView(viewName) {
        ...
        if (this.ui.workspaceFloatingActions) {
            this.ui.workspaceFloatingActions.style.display = (viewName === 'blocks') ? 'flex' : 'none';
        }
    }
    ...
    async loadWorkspaceFromCache() {
        const projects = await getAllProjects();
        let project = projects.find(p => p.name === this.projectName);

        if (!project) {
            console.log(`[IDE] Project "${this.projectName}" not found in DB. Creating new entry...`);
            project = {
            ...
            // Save metadata to IndexedDB immediately so it appears on Dashboard
            await saveProject(project);
        }
        ...
        if (project) {
            ...
            if (project.circuit) {
                const attemptCircuitLoad = () => {
                    // Check if Sim is ready AND Board Image has actual width (loaded)
                    if (this.simulatorManager.isReady && 
                        this.simulatorManager.boardElement && 
                        this.simulatorManager.boardElement.offsetWidth > 0) {
                        
                        this.simulatorManager.loadCircuitState(project.circuit);
                    } else {
                        // Keep waiting if image hasn't rendered yet
    ...
    async openFile(filePath) {
        ...
        if (isBlockly) {
            this.isCodeOnlyMode = false;
            if (!this.blocklyManager || !this.blocklyManager.workspace) {
                console.error("Blockly Workspace not initialized. Cannot open blocks.");
                return;
            }
    ...
    async uploadCodeToDevice() {
        ...
        try {
            ...
            if (this.currentCode.includes('import hcsr04') || this.currentCode.includes('from hcsr04')) {
                this.addConsoleMessage("📦 Auto-injecting 'hcsr04.py' (Manual Mode)...", "info");
                files['hcsr04.py'] = HCSR04_DRIVER_CODE;
            }
    ...
    runSimulation() {
        ...
        // --- FIX START: Global Function Extraction ---
        // We must identify ALL function names from BOTH setup and loop code
        // so that calls in the loop know they need to be awaited.
        const allFuncNames = [];
        const defRegex = /def\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(/g;
        const combinedSource = (codeObj.setupCode || '') + '\n' + (codeObj.loopCode || '');
        
        let match;
        while ((match = defRegex.exec(combinedSource)) !== null) {
            allFuncNames.push(match[1]);
        }
        // --- FIX END ---

        const prepareAsyncCode = (code) => {
            // 1. Convert sleep calls to await expressions
            let newCode = code
                .replace(/time\.sleep\(([^)]+)\)/g, "await _sim_sleep($1)")
                .replace(/time\.sleep_ms\(([^)]+)\)/g, "await _sim_sleep_ms($1)")
                .replace(/from hcsr04 import HCSR04/g, '');

            // 2. Convert all function definitions to async
            newCode = newCode.replace(/def\s+/g, "async def ");

            // 3. Inject 'await' before calling any of the user-defined functions
            // We now use the 'allFuncNames' array we populated earlier
            allFuncNames.forEach(name => {
                // Regex: Match function call, but NOT if it's preceded by "async def " (the definition itself)
                // or if it's already awaited.
                const callRegex = new RegExp(`(?<!async\\s+def\\s+)(?<!await\\s+)\\b${name}\\(`, 'g');
                newCode = newCode.replace(callRegex, `await ${name}(`);
            });
    ...
    setupWorkspaceListeners() {
        ...
        ws.addChangeListener((e) => {
            ...
            // 3. Trigger Orphan Manager (Shadow Blocks Logic)
            // We run this on structural changes to update the visual state of blocks
            if (e.type === Blockly.Events.BLOCK_MOVE ||
                e.type === Blockly.Events.BLOCK_CREATE ||
                e.type === Blockly.Events.BLOCK_DELETE ||
                e.type === Blockly.Events.FINISHED_LOADING) {

                if (this.orphanManager) {
                    // Small timeout ensures Blockly DOM is ready before we apply CSS classes
                    setTimeout(() => this.orphanManager.runOrphanCheck(), 10);
                }
            }
    ...
    async loadTutorialFromUrl(filename) {
        try {
            this.addConsoleMessage(`📚 Fetching tutorial: ${filename}...`, 'info');

            // Fetch from the public/tutorials/ directory
            ...
            // Wait a moment for the UI to fully settle
            setTimeout(() => {
                if (this.tutorialManager) {
                    this.tutorialManager.loadTutorial(json);
                    this.addConsoleMessage(`✅ Tutorial "${json.meta.title}" loaded.`, 'success');
                } else {
                    console.error("Tutorial Manager not found.");
                }
            }, 1000);

        } catch (e) {
            console.error("Tutorial Load Error:", e);
            this.addConsoleMessage(`❌ Failed to load tutorial: ${e.message}`, 'error');
            this.toastManager.show("Could not load tutorial content.", "error");
        }
    }
