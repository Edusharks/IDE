...
class PushButtonModel extends VirtualComponent {
    ...
    getInternalConnections(pinId) {
        if (this.isPressed) {
            if (pinId === 'SIG') return ['GND'];
            if (pinId === 'GND') return ['SIG'];
        }
        return [];
    }
...
class OLEDModel extends VirtualComponent {
    constructor(id, simulatorManager, position, name, componentData) {
        ...
        // Buffers - using your working approach
    ...
    render(container) {
            ...
            <svg viewBox="0 0 280 280" class="sim-oled-svg" style="width:100%; height:100%; position:absolute; top:0; left:0; z-index: 1; pointer-events: none;">
                <defs>
                    ...
                    <linearGradient id="glassReflect_${gid}" x1="0%" y1="0%" x2="100%" y2="100%">
                        ...
                        <stop offset="50%" style="stop-color:#fff;stop-opacity:0.08" />
                        <stop offset="51%" style="stop-color:#fff;stop-opacity:0.15" />
                ...
                <g transform="translate(30, 48)">
                    <rect x="0" y="0" width="220" height="155" fill="#000" stroke="#111" stroke-width="1" />
                    <!-- Active Area Black Background -->
                    <rect x="5" y="18" width="210" height="120" fill="#000000" stroke="#222" stroke-width="1" />
                    <path d="M 0,0 L 140,0 L 0,140 Z" fill="url(#glassReflect_${gid})" style="pointer-events:none;" />
                    <rect x="0" y="138" width="220" height="17" fill="#1a1a1a" opacity="0.8" />
                </g>

                <!-- Ribbon Cable -->
                <g transform="translate(80, 203)">
                    <path d="M 10,0 L 110,0 L 100,35 L 20,35 Z" class="oled-ribbon" />
                    <rect x="25" y="30" width="70" height="8" fill="#eee" rx="1" />
                    <g class="oled-ribbon-lines"><line x1="30" y1="0" x2="35" y2="35" /><line x1="45" y1="0" x2="48" y2="35" /><line x1="60" y1="0" x2="60" y2="35" /><line x1="75" y1="0" x2="72" y2="35" /><line x1="90" y1="0" x2="85" y2="35" /></g>
                </g>
            </svg>
        `;
        ...
        // Create the canvas for OLED display - using your working approach
        this.canvas = document.createElement('canvas');
        this.canvas.width = 128;
        this.canvas.height = 64;
        
        // Position it exactly over the OLED screen area in the SVG
        Object.assign(this.canvas.style, {
            position: 'absolute',
            top: '66px',    // Matches the SVG screen area (48 + 18)
            left: '35px',   // Matches the SVG screen area (30 + 5)  
            width: '210px',
            height: '120px',
            zIndex: '10',
        ...
        // Initialize with your working reset method
        this.reset();
    ...
    // --- YOUR WORKING DRAWING METHODS ---

    reset() {
        console.log(`[OLED-${this.id}] Reset called`);
        // Clear buffer
        this.bctx.fillStyle = this.bgColor;
    ...
    fill(col) {
        console.log(`[OLED-${this.id}] fill: ${col}`);
        this.bctx.fillStyle = (col > 0) ? this.pixelColor : this.bgColor;
        this.bctx.fillRect(0, 0, 128, 64);
    }
    ...
    text(str, x, y, col) {
        console.log(`[OLED-${this.id}] text: "${str}" at ${x},${y} color:${col}`);
        this.bctx.fillStyle = (col > 0) ? this.pixelColor : this.bgColor;
    ...
    line(x1, y1, x2, y2, col) {
        this.bctx.strokeStyle = (col > 0) ? this.pixelColor : this.bgColor;
    ...
    rect(x, y, w, h, col, fill) {
        const color = (col > 0) ? this.pixelColor : this.bgColor;
        if (fill) {
            this.bctx.fillStyle = color;
            this.bctx.fillRect(Math.floor(x), Math.floor(y), Math.floor(w), Math.floor(h));
        } else {
            this.bctx.strokeStyle = color;
            this.bctx.strokeRect(Math.floor(x) + 0.5, Math.floor(y) + 0.5, Math.floor(w), Math.floor(h));
        }
    }

    show() {
        console.log(`[OLED-${this.id}] show called`);
        // Transfer buffer to visible canvas - YOUR WORKING APPROACH
        if (this.ctx) {
    ...
    power(s) {
        console.log(`[OLED-${this.id}] power: ${s}`);
    }
    
    contrast(c) {
        console.log(`[OLED-${this.id}] contrast: ${c}`);
    }
    
    invert(i) {
        console.log(`[OLED-${this.id}] invert: ${i}`);
        // Simple inversion - flip colors
        if (i) {
            this.pixelColor = '#000000';
            this.bgColor = '#00FFFF';
        } else {
            this.pixelColor = '#00FFFF';
            this.bgColor = '#000000';
        }
    }
    
    getInternalConnections(id) { 
        return []; 
    }
    ...
    step(dt) {
        // Optional: Implement step if needed
    }
...
renderComponentProperties(component) {
    ...
    deleteSelectedWire() {
        ...
        if (wireIndex > -1) {
            ...
            //console.log(`Deleted wire ${this.selectedWireId}`);
            this.deselectWire();
            this._requestSaveState();
        }
    }
    ...
    loadCircuitState(state) {
        ...
        this.nextWireId = 1;
            ...
            switch (compState.type) {
                ...
                case 'soil_moisture':
                    ...
                    component.moistureLevel = compState.moistureLevel || 0.5;
                    break;
                case 'led_red':
            ...
            if (compState.temp !== undefined) component.temp = compState.temp;
            if (compState.humidity !== undefined) component.humidity = compState.humidity;
            if (compState.moistureLevel !== undefined) component.moistureLevel = compState.moistureLevel;
            if (compState.ledColor) component.ledColor = compState.ledColor;
            if (compState.red !== undefined) component.color = [compState.red, compState.green, compState.blue];
            if (compState.numPixels) component.setPixelCount(compState.numPixels);
            ...
            if (compState.type === 'dht11') {
                const tSlider = component.element.querySelector('.temp'); const hSlider = component.element.querySelector('.hum'); const tText = component.element.querySelector('.val-temp'); const hText = component.element.querySelector('.val-hum');
            ...
            if (compState.type === 'led_rgb') { setTimeout(() => component.updateVisualState(), 0); }
        });
        ...
        if (state.wires) {
            state.wires.forEach(wireState => {
                ...
                path.setAttribute('d', this.buildWirePath(wireState.points));
                this.wireSvgLayer.appendChild(path);

                const finalWire = { ...wireState, pathElement: path };
                
                finalWire.pathElement.addEventListener('click', (e) => { e.stopPropagation(); this.selectWire(finalWire, e); });
                finalWire.pathElement.addEventListener('dblclick', (e) => { e.stopPropagation(); this.addBendPointToWire(finalWire, e); });
        ...
        //console.log(`Circuit loaded: ${this.components.size} components, ${this.wires.length} wires. Next Wire ID: ${this.nextWireId}`);
