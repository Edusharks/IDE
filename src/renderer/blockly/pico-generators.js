...
micropythonGenerator.forBlock['actuator_neopixel_set'] = function(block) {
    ...
    const color = micropythonGenerator.valueToCode(block, 'COLOR', micropythonGenerator.ORDER_ATOMIC) || "'#000000'";
    const rgbFunc = getRgbHelper();
    const brightnessFunc = getBrightnessHelper();
...
micropythonGenerator.forBlock['wifi_start_web_server'] = function(block) {
    ...
    const functionName = 'start_web_server';
    if (!micropythonGenerator.functionNames_[functionName]) {
        const func = `
# --- Global Web Server State ---
_web_request_handler = None
_web_response_content = "<h1>Pico Not Programmed</h1>"
_web_request_path = "/"
...
def _web_server_thread():
    ...
    while True:
        try:
            ...
            # print('Client connected from', addr) # Optional: uncomment for more debug info
            ...
            path_start = request_str.find(' ') + 1
            path_end = request_str.find(' ', path_start)
            _web_request_path = request_str[path_start:path_end] if path_end > path_start else "/"
            # print("Request Path:", _web_request_path) # Optional: uncomment for more debug info

            # REFACTOR: Call the handler directly instead of in a new thread.
            # This is simpler and more reliable on a microcontroller.
            if callable(_web_request_handler):
                _web_request_handler()

            cl.send('HTTP/1.0 200 OK\\r\\nContent-type: text/html\\r\\n\\r\\n')
            cl.send(_web_response_content)
            cl.close()
        # FIX 2: Catch any exception, not just OSError, to prevent silent crashes.
        except Exception as e:
...
micropythonGenerator.forBlock['wifi_on_web_request'] = function(block) {
    ...
    const funcName = micropythonGenerator.nameDB_.getDistinctName(
        'on_web_request', Blockly.Names.NameType.PROCEDURE);
    
    // REFACTOR: The handler function is now much simpler. It doesn't need the try/finally or the global flag.
...
micropythonGenerator.forBlock['wifi_send_web_response'] = function(block) {
    const html = micropythonGenerator.valueToCode(block, 'HTML', micropythonGenerator.ORDER_ATOMIC) || '""';
    return `_web_response_content = str(${html})\n`;
};

micropythonGenerator.forBlock['wifi_get_web_request_path'] = function(block) {
    return ['_web_request_path', micropythonGenerator.ORDER_ATOMIC];
};
