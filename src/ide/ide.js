...
`;
    ...
    runSimulation() {
        ...
        const prepareAsyncCode = (code) => {
            // 1. Identify all user-defined function names
            const funcNames = [];
            ...
            // 2. Convert sleep calls to await expressions
            let newCode = code
                .replace(/time\.sleep\(([^)]+)\)/g, "await _sim_sleep($1)")
                .replace(/time\.sleep_ms\(([^)]+)\)/g, "await _sim_sleep_ms($1)")
                .replace(/from hcsr04 import HCSR04/g, '');

            // 3. Convert all function definitions to async
            newCode = newCode.replace(/def\s+/g, "async def ");

            // 4. Inject 'await' before calling any of the user-defined functions
            // We use a Negative Lookbehind (?<!async\s+) to ensure we don't double-await the definition line
            funcNames.forEach(name => {
                const callRegex = new RegExp(`(?<!async\\s+def\\s+)\\b${name}\\(`, 'g');
